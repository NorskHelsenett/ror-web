...
function SidebarProvider({
  ...
  const [cookiePending, setCookiePending] = React.useState(false)
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      size='icon'
      className={cn(
        'w-7 h-7 shrink-0 cursor-pointer hover:bg-background-hover flex items-center justify-center justify-self-end rounded-sm border border-subtle',
        className
      )}
      onClick={(event) => {
        onClick?.(event)
