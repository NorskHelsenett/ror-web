...
import { useReadReleases } from '@/hooks/use-read-releases'
...
export const ReleaseCard = ({ view }: { view: 'dashboard' | 'release-notes' }) => {
  ...
  const { markRead, isRead } = useReadReleases()
  ...
  return (
    <div className='bg-(--r-layer) rounded-lg px-6 pt-5 pb-2 w-full'>
      ...
      {!collapsed && (
        <div className='flex flex-col'>
          {(view === 'dashboard' ? releases.slice(0, 4) : releases).map((release, i) => {
            const read = release.id ? isRead(release.id) : false
            return (
              <Link
                key={i}
                ...
                className={cn(
                  'flex items-center gap-4 py-4 group -mx-6 px-6 transition-colors border-t border-border',
                  read ? 'hover:bg-muted/5 opacity-50' : 'hover:bg-muted/5'
                )}
              >
                {/* Unread indicator */}
                <span
                  className={cn(
                    'self-stretch w-0.5 rounded-full -ml-6 mr-2 shrink-0 transition-colors',
                    read ? 'bg-transparent' : 'bg-cyan-500'
                  )}
                />
      ...
      {view === 'dashboard' &&
        !collapsed &&
        (() => {
          const hiddenUnreadCount = releases.slice(4).filter((r) => r.id && !isRead(r.id)).length
          return (
            <Link
              href={routes.app.releaseNotes.getHref()}
              className='flex items-center justify-center gap-2 py-3 mt-1 border-t border-border text-sm text-muted-foreground hover:text-foreground transition-colors'
            >
              All releases
              {hiddenUnreadCount > 0 && (
                <span className='inline-flex items-center justify-center rounded-full bg-cyan-500 text-black text-[10px] font-semibold size-4'>
                  {hiddenUnreadCount}
                </span>
              )}
